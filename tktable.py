...
try:
    ...
    from tkinter import Frame, BOTH, N, E, S, W, CENTER, Entry, Canvas, Label,Button
...
class Multicolumn_Listbox(object):
    ...
    def insert_row(self, data, index=None,iid=None):
        ...
        if iid is None:
            ...
            item_ID = self.interior.insert('', index,iid=iid, values=data)
        self.interior.item(item_ID, tags=item_ID)
        print("addded item with id",item_ID)
...
class Tk_Table(Frame, object):
    def __init__(self, master, columns, data=None, command=None, editable=True, sort=True, select_mode=None,
        ...
        self._multicolumn_listbox.interior.bind("<Motion>", self.on_hover)
    ...
    def on_hover(self,event):
        ...
        #print("cell data on hover is",cell_data)
        if self.last_focus == (item_ID,column_number):
            return
        self.last_focus = (item_ID, column_number)
        # close previous popups
        if self._btn_popup:
            self._btn_popup.destroy()
        if column_number > 3:
            return
        self._btn_popup = Button(self._multicolumn_listbox.interior,text = cell_data,bg="light blue",command= lambda :self.popup_clicked(column_number,item_ID))
        self._btn_popup.place(x=x, y=y, width=width, height=height)

    def popup_clicked(self,col,iid):
        if self.callback:
            self.callback(col,iid)
    ...
    def insert_row(self, data, index=None,iid=None):
        print("called insert row from here 1")
        self._multicolumn_listbox.insert_row(data, index,iid=iid)
        if self._row_numbers:
